...
- Do not mention that you are an AI model.`;function ut(){return typeof LanguageModel<"u"}async function R(){if(!ut())return{status:"unsupported",message:"Ürün asistanı bu tarayıcıda desteklenmiyor. Güncel Chrome masaüstü sürümünü deneyin."};try{const e=await LanguageModel.availability(H);return e==="unavailable"?{status:"unavailable",message:"Ürün asistanı şu an cihazınızda kullanılamıyor. Chrome güncellemesini ve yeterli depolama alanını kontrol edin."}:{status:e,message:null}}catch(e){return{status:"error",message:(e==null?void 0:e.message)||"Asistan şu an kullanılamıyor."}}}class ht{constructor(){y(this,w,null);y(this,P,"")}async prepare(t,{onDownloadProgress:s}={}){const n=await R();if(n.status==="unsupported"||n.status==="unavailable"){const i=new Error(n.message);throw i.code=n.status,i}return m(this,P,dt(t)),m(this,w,await LanguageModel.create({...H,initialPrompts:[{role:"system",content:lt},{role:"user",content:`Here are the only facts you may use:
